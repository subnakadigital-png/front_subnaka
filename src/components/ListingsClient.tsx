...
const AdSidebar = () => {
    const router = useRouter();
    return (
    <aside className="w-full lg:w-1/5 h-fit sticky top-24 space-y-6">
        <PostPropertyBanner />
...
export default function ListingsClient({ properties }: { properties: Property[] }) {
  const [filters, setFilters] = useState({
    keyword: '',
    type: 'All', // For Sale/Rent
    propertyType: 'All', // For House/Apartment
    location: 'All',
    bedrooms: 'Any',
  ...
  const propertiesToDisplay = useMemo(() => {
    let filtered = properties.filter((p: Property) => {
        ...
        const propertyTypeMatch = filters.propertyType === 'All' || p.propertyType === filters.propertyType;
          ...
          <aside className="hidden lg:block w-full lg:w-1/4 h-fit sticky top-24">
              <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                  <FilterContent 
                    ...
                    locations={locations} 
                    propertyTypes={propertyTypes} // This is now correct
                  />
              </div>
          </aside>
          ...
          <main className="w-full lg:w-3/5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {propertiesToDisplay.map((p: Property) => (
                <PublicPropertyCard key={p.id} prop={p} />
              ))}
